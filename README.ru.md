...
**91 tok/s. Ноль контейнеров. 123 ГБ памяти GPU. Скомпилировано из исходников. Я знаю кунг-фу.**
...
- **Контейнеры добавляют 15–20% накладных расходов** на GPU-задачах. Когда у вас 123 ГБ unified-памяти на одном чипе, каждый ватт и каждый байт должны идти на инференс, а не на оркестрацию. *«Не пытайся согнуть ложку. Вместо этого попытайся осознать истину: ложки нет.»*
- **Компиляция из исходников** означает нативные оптимизации gfx1151, которые пропускают готовые бинарники. Вот откуда берутся 91 tok/s.
...
- **Генерация изображений** — [ComfyUI](https://github.com/comfyanonymous/ComfyUI) на 123 ГБ GPU, SDXL, Flux
...
|--------|----------|------|
| Qwen3-30B-A3B (MoE) | **83-91 tok/s** | 18 ГБ |
...
| 6 | Lemonade + llama.cpp — единый API, 91 tok/s | скоро |
...
| 9 | Генерация изображений — ComfyUI на 123 ГБ GPU | скоро |
